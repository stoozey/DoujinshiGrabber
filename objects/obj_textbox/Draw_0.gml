draw_9slice(x, y, image_xscale, image_yscale, spr_9slice_button, 0, c_white);

draw_set_font(fnt_textbox);
draw_set_alpha(image_alpha + (enabled / 10));
	draw_set_colour(global.textCol2);
		var _typebeam, _txtScale, _text, _buff;
		_typebeam = ((cursor.selection == id) ? typebeam : "");
		_txtScale = [1, 1];
		_text = text + _typebeam;
		_buff = (isTags) ? 64+20 : 0;
		
		while ((string_width(_text) * _txtScale[X]) > (image_xscale - _buff))
			_txtScale[X] -= 0.001;
		
		while ((string_height(text) * _txtScale[Y]) > image_yscale)
		{
			_txtScale[Y] -= 0.001;
			_txtScale[X] -= 0.001;
		}
		
		draw_text_transformed(x, y + 2, _text, _txtScale[X], _txtScale[Y], 0);
